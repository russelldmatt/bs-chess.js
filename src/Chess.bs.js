...
var all$1 = $$Array.to_list($$Array.init(8, (function (x) {
            return x + 1 | 0;
          })));

var Rank = /* module */[/* all */all$1];
...
var include = Belt_Id.MakeHashable(T);
...
console.log(toRaw$4(full_move));

console.log(Belt_Option.map(move(chess$1, /* Full */Block.__(2, [full_move])), toRaw$4));
...
function play_random_game($staropt$star, t) {
  var print = $staropt$star ? $staropt$star[0] : /* false */0;
  ...
  while(true) {
    ...
    if (t$1.game_over()) {
      ...
      if (print !== 0) {
        console.log(toRaw$4(selected_move));
      }
...
play_random_game(/* Some */[/* true */1], create(/* None */0, /* () */0));

var endStates = Belt_HashMap.make(5, [
      include[0],
      include[1]
    ]);

function total(endStates) {
  return Belt_HashMap.reduce(endStates, 0, (function (count, _, v) {
                return count + v | 0;
              }));
}

function check_for_game_over_but_no_end_state(_n) {
  while(true) {
    var n = _n;
    if (n <= 0) {
      return /* () */0;
    } else {
      console.log(n, " rounds left");
      var chess = create(/* None */0, /* () */0);
      play_random_game(/* Some */[/* false */0], chess);
      var match = endState(chess);
      if (match) {
        var x = match[0];
        console.log(toString$5(x));
        var count = Belt_Option.getWithDefault(Belt_HashMap.get(endStates, x), 0);
        Belt_HashMap.set(endStates, x, count + 1 | 0);
        var total$1 = total(endStates);
        Belt_HashMap.forEach(endStates, (function(total$1){
            return function (endState, count) {
              console.log("--", toString$5(endState), count / total$1);
              return /* () */0;
            }
            }(total$1)));
        _n = n - 1 | 0;
        ...
        console.log(chess.ascii());
        return Js_exn.raiseError("game over but not sure why");
      }
    }
  };
}
